        ...
        for provider_model, mentions in providers.items():
            ...
            for query, query_mentions in query_groups.items():
                ...
                for mention in sorted(query_mentions, key=lambda x: x['rank']):
                    print(f"      #{mention['rank']} - {mention['alias_used']}")
...
def print_provider_comparison(db_path="llmseo.db"):
    ...
    for row in results:
        ...
        success_rate = (success_resp / total_resp * 100) if total_resp > 0 else 0
        avg_rank_str = f"{avg_rank:.1f}" if avg_rank else "N/A"
        
        print(f"{provider:<15} {model:<15} {mentions:<10} {avg_rank_str:<10} {success_rate:.1f}%")
...
def main():
    ...
    parser.add_argument('--report', action='store_true', help='Show brand report')
    parser.add_argument('--compare', action='store_true', help='Show provider comparison')
