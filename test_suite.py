...
def test_database_operations():
    ...
    try:
        ...
        if all(table in tables for table in expected_tables):
            ...
            return False
        test_brand = {"id": 1, "name": "TestBrand", "aliases": ["TB", "Test Brand"]}
...
def test_configuration():
    ...
    try:
        ...
        if "brands" in config and "queries" in config:
            print(f"Default config loaded with {len(config['brands'])} brands and {len(config['queries'])} queries")
        else:
        ...
        if os.path.exists("config.json"):
            ...
            print(f"Loaded config.json with {len(config['brands'])} brands and {len(config['queries'])} queries")
    ...
    try:
        ...
        try:
            ...
            if "answers" in result and result["answers"]:
                ...
                print(f"Integration test completed - found {mentions_found} brand mentions")
